...
class Renderer(Window):
    def __init__(self):
        super().__init__(920, 670, "Bubble Sort Visualization")
        self.batch = Batch()
        ...
        for e, i in enumerate(self.x):
            self.bars.append(Rectangle(50 + e * spacing, 50, 15, i * 10, color=(173, 216, 230), batch=self.batch))
    ...
    def update_bars(self):
        self.bars = []
        spacing = 13  # Adjust the spacing between bars
        for e, i in enumerate(self.x):
            self.bars.append(Rectangle(50 + e * spacing, 50, 15, i * 10, color=(173, 216, 230), batch=self.batch))
    def on_draw(self):
        self.clear()
